...
async function verifyState(secret: string, state: string): Promise<string | null> {
	...
	const payloadBytes = Uint8Array.from(atob(pad(payloadB64).replace(/-/g, "+").replace(/_/g, "/")), (c) => c.charCodeAt(0));
	const sigBytes = Uint8Array.from(atob(pad(sigB64).replace(/-/g, "+").replace(/_/g, "/")), (c) => c.charCodeAt(0));
	const ok = await crypto.subtle.verify("HMAC", key, sigBytes, payloadBytes);
	if (!ok) return null;
	const payload = new TextDecoder().decode(payloadBytes);
	const [nonce, expStr, nextPart] = payload.split(":");
...
function safeNext(raw: string | null): string {
	return raw === "checkout" || raw === "account" || raw === "cart" ? raw : "";
}

export async function buildAuthorizeUrl(c: Context<AppEnv>, provider: OAuthProvider): Promise<Response> {
	...
	const state = await signState(stateSecret, `${generateToken().slice(0, 16)}:${exp}:${next}`);
	const origin = getAppOrigin(c);
	const redirectUri = `${origin}/api/auth/oauth/${provider}/callback`;

	if (provider === "github") {
		const clientId = c.env.GITHUB_CLIENT_ID;
		if (!clientId) return c.json({ error: "GitHub OAuth is not configured (GITHUB_CLIENT_ID)" }, 503);
...
async function exchangeGitHub(c: Context<AppEnv>, code: string, redirectUri: string): Promise<OAuthProfile> {
	const clientId = c.env.GITHUB_CLIENT_ID;
	...
	const userRes = await fetch("https://api.github.com/user", {
		headers: { Authorization: `Bearer ${tokenData.access_token}`, Accept: "application/vnd.github+json", "User-Agent": "vite-react-template" },
	});
	...
	if (!email) email = `${ghUser.id}+${ghUser.login || "user"}@users.noreply.github.com`;
	return { provider: "github", providerUserId: String(ghUser.id), email: email.toLowerCase() };
}

async function exchangeGoogle(c: Context<AppEnv>, code: string, redirectUri: string): Promise<OAuthProfile> {
	const clientId = c.env.GOOGLE_CLIENT_ID;
	...
	const tokenData = (await tokenRes.json()) as { access_token?: string; error?: string };
	if (!tokenData.access_token) throw new Error(tokenData.error || "Failed to get Google access token");
...
export async function upsertOAuthUser(db: D1Database, profile: OAuthProfile): Promise<{ token: string; user: { id: number; email: string } }> {
	...
	let user = await db.prepare("SELECT id, email FROM users WHERE email = ?").bind(profile.email).first<{ id: number; email: string }>();
	if (!user) {
		const unusable = await hashPassword(generateToken());
		user = await db.prepare("INSERT INTO users (email, password_hash) VALUES (?, ?) RETURNING id, email").bind(profile.email, unusable).first<{ id: number; email: string }>();
		if (!user) throw new Error("Failed to create user");
	}
	await db.prepare("INSERT INTO oauth_accounts (user_id, provider, provider_user_id) VALUES (?, ?, ?)").bind(user.id, profile.provider, profile.providerUserId).run();
	const token = await createSession(db, user.id);
	return { token, user };
}

export async function handleOAuthCallback(c: Context<AppEnv>, provider: OAuthProvider): Promise<Response> {
	const url = new URL(c.req.url);
	const err = url.searchParams.get("error");
	if (err) return c.redirect(`/?auth_error=${encodeURIComponent(err)}`, 302);
	const code = url.searchParams.get("code");
	const state = url.searchParams.get("state");
	if (!code || !state) return c.redirect("/?auth_error=missing_code", 302);
	const stateSecret = c.env.OAUTH_STATE_SECRET || c.env.GITHUB_CLIENT_SECRET || "dev-oauth-state-secret";
	const valid = await verifyState(stateSecret, state);
	if (!valid) return c.redirect("/?auth_error=invalid_state", 302);
	const nextQs = safeNext(valid) ? `&next=${encodeURIComponent(safeNext(valid))}` : "";
	const origin = getAppOrigin(c);
	const redirectUri = `${origin}/api/auth/oauth/${provider}/callback`;
	try {
